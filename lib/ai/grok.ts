...
export interface GrokEvaluationRequest {
  question: string
  answer: string
  rubric: {
    criteria: Array<{
...
export interface GrokEvaluationResponse {
  scores: Array<{
    criterion: string
...
export async function evaluateWithGrok(
  apiKey: string,
  request: GrokEvaluationRequest
): Promise<GrokEvaluationResponse> {
      ...
      'https://api.x.ai/v1/chat/completions',
      {
        ...
        body: JSON.stringify({
          messages: [
            {
              role: 'system',
              content: '당신은 교육 평가 전문가입니다. 학생의 답변을 루브릭 기준에 따라 정확하고 건설적으로 평가합니다.'
            },
            {
              role: 'user',
              content: prompt
...
function buildEvaluationPrompt(request: GrokEvaluationRequest): string {
...
`
  ...
  return `다음 학생의 답변을 주어진 루브릭 기준으로 평가해주세요.
...
function parseEvaluationResponse(
  text: string,
  request: GrokEvaluationRequest
): Omit<GrokEvaluationResponse, 'evaluatedAt'> {
  try {
